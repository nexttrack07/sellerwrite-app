...
.validator((d: unknown) => {
  return z.union([
    ...
    // Just update keywords for an existing description
    z.object({
      id: z.number(),
...
.handler(async ({ data }) => {
  ...
  if ('id' in data) {
    // Just update the keywords_used field for an existing description
    ...
    if (updateError) {
      throw new Error(`Failed to update description keywords: ${updateError.message}`);
    }
